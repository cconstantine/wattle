...
describe GroupingWatNotifier do
  let(:grouping_notifier) {GroupingWatNotifier.new(grouping)}
    ...
    context "on a grouping that never notified" do
      ...
      context "When multiple workers run at the same time" do
        it "should only call send_email once" do
          ...
          any_instance_of(GroupingWatNotifier) do |klass|
          ...
          3.times do |i|
            threads << Thread.new do
              GroupingWatNotifier.notify(grouping.id)
            end
          end
